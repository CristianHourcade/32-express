...
/* ────────── Helper: resaltar “added/removed” ────────── */function highlight(text: string) {
  return text.replace(/(added|removed|quitó|agregó)/gi, (m) => {
    const lower = m.toLowerCase();
    const isAdd = lower === "added" || lower === "agregó";
    const colorClass = isAdd
      ? "text-emerald-600"
      : "text-rose-600";
    return `<span class="${colorClass} font-semibold">${m}</span>`;
  });
}
