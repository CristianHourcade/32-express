...
// Impuesto bancario (5% sobre todo lo que no es efectivo)
const BANK_TAX_RATE = 0.05;
...
function StatCard({
  ...
  return (
    <div
      className={`rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm ${className}`}
    >
...
> = {
  ...
  card: {
    label: "Tarjeta (incl. Transfer/MP)",
    short: "TJ",
...
function BusinessCard({
  ...
  const saving:
    | { today: number; yesterday: number; mtd: number }
    | undefined = b.savingTargets;
  ...
  function KPI({
    ...
    return (
      <div className="min-w-[150px]">
        ...
        <div
          className={`mt-1 text-sm font-semibold tabular-nums whitespace-nowrap leading-tight ${className}`}
        >
          {value}
        </div>
      </div>
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {segments
            ...
            .map((s) => (
              ...
              >
                <span
                  className={`inline-block w-2 h-2 rounded ${s.dotClass}`}
                />
            ...
            <div className="sm:col-span-3 mt-1 overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {UNIFIED_KEYS.map((k) => {
                    const payments = unifyPayments(b.paymentMethods || {});
                    const expensesBy = unifyExpenses(
                      b.expensesByMethod || {}
                    );
                      ...
                      >
                        <td className="py-1">
                          <span className="inline-flex items-center gap-2">
                            <span
                              className={`w-2 h-2 rounded ${METHOD_META_UNI[k].dotClass}`}
                            />
...
function ShiftCard({
  ...
  return (
    <div
      className="group relative overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm hover:shadow-md transition-all cursor-pointer"
      onClick={onOpenDetails}
      onKeyDown={(e) =>
        (e.key === "Enter" || e.key === " ") && onOpenDetails?.()
      }
    >
          ...
          {items.map((it) => {
            ...
            return (
              <div
                ...
                className={`${it.pill} ${
                  visible ? "" : "invisible"
                } rounded-xl px-3 py-2 flex items-center justify-between text-sm border border-slate-200/70 dark:border-slate-700/60`}
              >
                <span className="flex items-center gap-2">
                  <span
                    className={`w-2 h-2 rounded ${it.dot}`}
                  />
...
type BusinessBalance3M = {
  businessId: string;
  businessName: string;
  income: number; // neto (cash + banco neto)
  expense: number;
  balance: number;
  incomeCash: number;
  incomeRest: number; // ya neto de impuesto
  expenseCash: number;
  expenseRest: number;
  balanceCash: number;
  balanceRest: number;
...
export default function AdminDashboard() {
  ...
  // Balance global histórico (no 3M)
  ...
  // Expansión por negocio (en vez de abrir todos juntos)
  const [expandedBusinessId, setExpandedBusinessId] = useState<string | null>(
    null
  );

  /* ───────── 3 MESES: Balance por negocio (con EF / BANCO neto de 5%) ───────── */
  useEffect(() => {
    let mounted = true;
    (async () => {
      ...
      const salesBatches = await Promise.all(
        ids.map((id) => loadSales(id, start, end))
      );
      const salesData = salesBatches.flat();

      const expenseBatches = await Promise.all(
        ids.map((id) => loadExpenses(id, start, end))
      );
  ...
  const { perBiz3m, global3m } = useMemo(() => {
    ...
    const bizMap = new Map<
      string,
      {
        name: string;
        incomeCash: number;
        incomeRest: number; // bruto antes de impuesto
        expenseCash: number;
        expenseRest: number;
      }
    >();
    ...
    for (const s of sales3m) {
      ...
      if (String(s?.payment_method) === "cash") {
        incomeCash += t;
        const obj = bizMap.get(id)!;
        obj.incomeCash += t;
      } else {
        incomeRestRawGlobal += t;
        const obj = bizMap.get(id)!;
        obj.incomeRest += t;
      }
    }
    ...
    for (const e of expenses3m) {
      ...
      if (String(e?.method) === "cash") {
        expenseCash += a;
        bizMap.get(id)!.expenseCash += a;
      } else {
        bizMap.get(id)!.expenseRest += a;
      }
    }

    // Ajuste BANCO (resto): aplica 5% sobre todo lo que no es efectivo
    const incomeRestNetGlobal = incomeRestRawGlobal * (1 - BANK_TAX_RATE);
    const totalIncomeNet = incomeCash + incomeRestNetGlobal;

    let perBiz: BusinessBalance3M[] = Array.from(
      bizMap.entries()
    ).map(([businessId, v]) => {
      const incomeRestNet = v.incomeRest * (1 - BANK_TAX_RATE);
      const incomeNet = v.incomeCash + incomeRestNet;
      const expenseTotal = v.expenseCash + v.expenseRest;
      ...
      return {
        ...
        balance: incomeNet - expenseTotal,
        incomeCash: v.incomeCash,
        incomeRest: incomeRestNet,
        expenseCash: v.expenseCash,
        expenseRest: v.expenseRest,
        balanceCash: v.incomeCash - v.expenseCash,
        balanceRest: incomeRestNet - v.expenseRest,
      };
    });

    // fallback sin movimientos
    if (perBiz.length === 0 && groupBusinesses.length > 0) {
      perBiz = groupBusinesses.map(
        (b: any): BusinessBalance3M => ({
          businessId: b.id,
          businessName: b.name,
          income: 0,
          expense: 0,
    ...
    const expenseRestGlobal = Math.max(0, expense - expenseCash);

    return {
      perBiz3m: perBiz,
      global3m: {
        income: totalIncomeNet, // ya neto de impuesto bancario
        expense,
        balance: totalIncomeNet - expense,
        incomeBy: { cash: incomeCash, rest: incomeRestNetGlobal },
        expenseBy: { cash: expenseCash, rest: expenseRestGlobal },
        balanceBy: {
          cash: incomeCash - expenseCash,
          rest: incomeRestNetGlobal - expenseRestGlobal,
        },
        loading: loading3m,
      },
    };
  }, [businesses, sales3m, expenses3m, loading3m]);
  ...
  /* ───────── Carga de mes seleccionado (cards de negocios + turnos) ───────── */
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
  ...
  /* ───────── GLOBAL HISTÓRICO (para referencia general) ───────── */
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        ...
        console.error(err);
        if (mounted)
          setGlobalBalance((g) => ({ ...g, loading: false }));
      }
    })();
    return () => {
      mounted = false;
  ...
  const openShiftProducts = async (sh: any) => {
    ...
    const totalTurn = turnSales.reduce(
      (a, s) => a + (s.total ?? 0),
      0
    );
    const saleIds = turnSales.map((s) => s.id);
    const items = saleIds.length
      ? await loadSaleItemsPorSaleIds(saleIds)
      : [];
  ...
  const topProducts = useMemo(() => {
    ...
    const recent = directSales.filter(
      (s) =>
        (now - new Date(s.timestamp).getTime()) / 86400000 <=
        daysFilter
    );
  ...
  const businessesWithMonthlyData = useMemo(() => {
    ...
    const elapsedMillis = Math.max(
      0,
      Math.min(now.getTime(), monthEnd.getTime()) -
        monthStart.getTime()
    );
    ...
    for (const s of sales) {
      ...
      if (ts >= todayISOStart && ts < todayISOEnd)
        d.todayAmount += s.total;
      if (ts >= yISOStart && ts < yISOEnd)
        d.yesterdayAmount += s.total;
    }
    ...
    return businesses.map((b) => {
      ...
      const savingYesterday = savingTargetForDayAmount(
        d.yesterdayAmount
      );
  ...
  const calcShiftTotals = (sh: any) => {
    ...
    ss.forEach((s) => {
      if (s.payment_method in pm)
        (pm as any)[s.payment_method] += s.total;
    });
  ...
  const monthLabel = useMemo(
    () =>
      monthStart.toLocaleString("es-ES", {
        month: "long",
        year: "numeric",
      }),
    [monthStart]
  ...
  /* ───────────── RENDER ───────────── */
  const totalIncome3M_Cash = rows3m.reduce(
    (a, x) => a + x.incomeCash,
    0
  );
  const totalIncome3M_Rest = rows3m.reduce(
    (a, x) => a + x.incomeRest,
    0
  );
  const totalExpense3M_Cash = rows3m.reduce(
    (a, x) => a + x.expenseCash,
    0
  );
  const totalExpense3M_Rest = rows3m.reduce(
    (a, x) => a + x.expenseRest,
    0
  );
  const totalBalance3M_Cash = rows3m.reduce(
    (a, x) => a + x.balanceCash,
    0
  );
  const totalBalance3M_Rest = rows3m.reduce(
    (a, x) => a + x.balanceRest,
    0
  );
  ...
  return (
    <div className="space-y-8 p-4 md:p-6 max-w-7xl mx-auto">
      {/* ===== BALANCE GLOBAL (3 MESES) ===== */}
      <section>
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 md:p-5 shadow-sm">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
            <div>
              <h1 className="text-xl md:text-2xl font-bold tracking-tight text-slate-800 dark:text-white">
                Balance 3 meses · Grupo 1
              </h1>
              <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400">
                Ingresos netos (efectivo + banco con 5% descontado) y
                egresos por negocio.
              </p>
            </div>
            {loading3m && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <StatCard
              label="Ingresos netos (3 meses)"
              value={fmtMoney(global3m.income)}
            />
            <StatCard
              label="Egresos (3 meses)"
              value={
                <span className="text-red-600 dark:text-red-400">
                  {fmtMoney(global3m.expense)}
                </span>
              }
            />
            <StatCard
              label="Balance (3 meses)"
              value={
                <span
                  className={
                    global3m.balance >= 0
          ...
          <div className="mt-4 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
            <div className="flex items-center justify-between mb-2 gap-2">
              <div className="text-sm font-semibold">
                Balance por negocio (últimos 3 meses)
              </div>
              {global3m.loading && (
                <span className="text-xs px-2 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500">
                  Calculando…
              ...
              <table className="w-full text-sm">
                <thead className="text-[11px] text-slate-500">
                  <tr>
                    ...
                    <th className="text-right py-1">
                      Balance BANCO (neto 5%)
                    </th>
                    <th className="text-right py-1">Balance total</th>
                  </tr>
                </thead>
                <tbody>
                  {rows3m.map((r) => (
        ...
        <header className="flex items-center justify-between flex-wrap gap-4 mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Ventas, gastos y rentabilidad de cada sucursal en el
              mes seleccionado.
            </p>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activeShifts.map((sh) => {
            ...
            const emp = employees.find(
              (e) => e.id === sh.employee_id
            );
            const hours =
              (Date.now() - new Date(sh.start_time).getTime()) /
              36e5;
