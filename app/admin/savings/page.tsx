...
const loadBusinesses = async () => {
  ...
  /** Devuelve un query listo para paginar */
  const baseMoves = (bizId: string) =>
    supabase.from("savings_transactions").select("*").eq("business_id", bizId);
  
  /** Movimientos dentro de rango (mes) */
  ...
  /** TODOS los movimientos (saldo histórico) */
  const loadLifetime = async (bizId: string) =>
    paged((lo, hi) =>
      baseMoves(bizId)
        .order("created_at", { ascending: false })
        .range(lo, hi)
        .select("amount")            // sólo la columna amount para menos tráfico
    );
...
export default function SavingsPage() {
  ...
  const [moves, setMoves] = useState<any[]>([]);            // movimientos del mes
  const [lifetimeByBiz, setLifetimeByBiz] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const b of businesses) {
        // movimientos del mes
        monthMoves = monthMoves.concat(await loadMoves(b.id, start, end));

        // saldo histórico
        const all = await loadLifetime(b.id);
        lifetimeTotals[b.id] = all.reduce((a, n) => a + n.amount, 0);
      }

      setMoves(monthMoves);
      setLifetimeByBiz(lifetimeTotals);
      setLoading(false);
    })();
  }, [start, end]);
  ...
  const refreshBiz = async (id: string) => {
    const [month, all] = await Promise.all([loadMoves(id, start, end), loadLifetime(id)]);
    setMoves((prev) => prev.filter((m) => m.business_id !== id).concat(month));
    setLifetimeByBiz((prev) => ({ ...prev, [id]: all.reduce((a, n) => a + n.amount, 0) }));
  };
  ...
  const rows = biz.map((b) => {
    ...
    return { ...b, monthTotal, lifetime: lifetimeByBiz[b.id] ?? 0, list };
  });
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {rows.map((b) => (
              <div key={b.id}
                ...
                <p className={`text-2xl font-bold ${b.monthTotal >= 0 ? "text-green-600 dark:text-green-400" : "text-rose-500"} mb-2`}>
                  $ {fmt(Math.abs(b.monthTotal))} {b.monthTotal < 0 && "(–)"}
                </p>

                {/* Saldo histórico */}
                <p className="text-sm text-slate-500 dark:text-slate-400">Saldo histórico</p>
                <p className="text-lg font-semibold mb-4">
                  $ {fmt(Math.abs(b.lifetime))} {b.lifetime < 0 && "(–)"}
                </p>
