// Nueva pantalla para armar faltantes fácilmente
"use client";
...
export default function FaltantesPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const stockMap = new Map(inventory.map((i) => [i.product_id, i.stock]));

            const faltantesCalculados = masters.map((m) => {
                const vendido = ventaMap.get(m.id) ?? 0;
                const stock = stockMap.get(m.id) ?? 0;
                const needsInspection = stock === 0 && vendido > 0;
                const faltan = needsInspection ? vendido * 2 : vendido - stock;
                const categoria = m.name?.split(" ")[0]?.toUpperCase();

                return vendido > stock
                    ? {
                        id: m.id,
                        name: m.name,
                        vendido,
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="p-4 bg-white rounded-xl border space-y-2">
                <div className="flex gap-4 flex-wrap items-center">
                    ...
                    >
                        {copied ? <><Check size={16} /> Copiado</> : <><ClipboardCopy size={16} /> Copiar seleccionados</>}
                    </button>
                </div>
            </div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {f.needsInspection && (
                                                    <AlertTriangle
                                                        ...
                                                        title="Posible pérdida de ventas por falta de stock"
                                                    />
                                                )}
                                            </div>
                                        </td>
